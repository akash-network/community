...
Max: We also added the main Netflix support for GPU and USDC. So users cannot deploy GPUs. And also Usdc is a trickier to deploy with because you need to know the exact IBC denim to get bids from providers on the main net. So we'll address this and work to make it easier in the coming months. Because obviously, the usdc, deployments add a lot of value to the attachment work. And we also added the new towels on the dashboard and graphs for the GPUs. And also for the US or yet. Yeah. The US and the Usdc.
